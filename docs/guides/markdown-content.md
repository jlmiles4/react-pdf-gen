...
Anything not in this table generally falls through to body text, so links, images, horizontal rules, HTML comments, and markdown tables render as literal characters. Plain blockquotes and malformed or unsupported alert variants are the exception: the parser strips the leading `>` and renders the remaining text as ordinary body copy. Inline `**bold**`, `*italic*`, and `` `code` `` runs are recognized inside headings, paragraphs, list items, and callout bodies (underscore italics require non-alphanumeric boundaries, so `snake_case` identifiers are left alone).
...
- Loading authored content from `.md` files at build time (current pattern: one demo chapter split across two page components).
...
- **No ordered lists, no `####`.** `1. item` lines have no branch, so they'd merge into one paragraph — the parser emits a build-time `console.warn` when it sees them (use `-` bullets instead). `#### heading` degrades to a body paragraph with the `####` markers stripped, and the parser emits a `console.warn` naming the line; only `#`/`##`/`###` are headings.
