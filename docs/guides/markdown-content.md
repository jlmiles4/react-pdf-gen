...
One page: [`Page21-Ch12-MarkdownAutomation.tsx`](../../src/pages/Page21-Ch12-MarkdownAutomation.tsx). It reads `content/chapters/12-markdown-demo.md` at build time, strips a YAML-style frontmatter block, and passes the body to `<MarkdownRenderer>`:
...
Anything not in this table is ignored. There is no inline emphasis (`**bold**`, `_italic_`), no links, no images, no horizontal rules, no markdown tables.
