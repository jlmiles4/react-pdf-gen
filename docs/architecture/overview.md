...
3. **Components** (`src/components/`) — the building blocks every page composes: `ContentPage`, `ChapterTitle`, `SectionHeading`, `SectionBanner`, `TipBox`/`WarningBox`/`InfoBox`, `CodeBlock`, `BulletList`, `Table`, `RecipeCard`, `MarkdownRenderer`, `Header`, `Footer`, `Icons`. See [components reference](../reference/components.md).
