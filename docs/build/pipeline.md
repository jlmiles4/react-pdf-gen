...
`output/` is gitignored except the book PDF itself, which is tracked as the shipped deliverable. Everything in it can be regenerated by `pnpm pipeline`. A rebuild always changes the tracked PDF's timestamped metadata; compare extracted text with the committed PDF and restore metadata-only binary churn instead of committing it:
