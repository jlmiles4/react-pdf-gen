...
`pnpm export` shells out to `pdftoppm`. If it's missing, the script prints platform-specific `poppler-utils` install instructions (apt / brew / dnf) and exits — it never installs anything itself (see `scripts/export-pages.sh`). The build itself also needs `pdftotext` from the same package for the TOC two-pass.
